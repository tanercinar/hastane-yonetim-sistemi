...
public sealed class InpatientDashboardComponentTests : BunitContext
{
    private static readonly Guid WardId = Guid.Parse("00000000-0000-0000-0000-000000000010");
    private static readonly Guid DeptId = Guid.Parse("30000000-0000-0000-0000-000000000003");

    [Fact]
    public void InpatientDashboardRendersMetricsAndWardBreakdownCorrectly()
    {
        Services.AddSingleton<IInpatientApiClient>(new FakeDashboardInpatientApiClient());

        var cut = Render<InpatientDashboard>();

        cut.WaitForAssertion(() =>
        {
            Assert.Contains("Servis &amp; Yatak Doluluk Dashboard'u", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("Genel Doluluk Oranı", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("%75.0", cut.Markup, StringComparison.Ordinal);
            ...
            Assert.Contains("15", cut.Markup, StringComparison.Ordinal); // Occupied
        });
    }

    private sealed class FakeDashboardInpatientApiClient : IInpatientApiClient
    {
        ...
        {
            return Task.FromResult<InpatientDashboardResponse?>(new InpatientDashboardResponse(
                TotalBeds: 20,
                OccupiedBeds: 15,
                AvailableBeds: 3,
                CleaningBeds: 1,
                MaintenanceBeds: 1,
                OverallOccupancyPercentage: 75.0,
                PendingAdmissionsCount: 2,
                PendingTransfersCount: 1,
                TodayDischargesCount: 5,
                Wards:
                [
                    new WardOccupancySummaryItem(
                        WardId,
                        "Kardiyoloji Servisi",
                        "DEMO-WRD-CARD",
                        DeptId,
