...
public sealed class PatientDiagnosticTimelineComponentTests : BunitContext
{
    private readonly FakeDiagnosticsApiClient _fakeApi = new();

    public PatientDiagnosticTimelineComponentTests()
    {
    ...
    public void PatientDiagnosticTimelineRendersEventsAndParameters()
    {
        var patientId = Guid.NewGuid();
        var cut = Render<PatientDiagnosticTimeline>(parameters => parameters.Add(p => p.PatientId, patientId));

        cut.WaitForAssertion(() =>
        {
            Assert.Contains("Biyokimya Panel", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("Laboratuvar", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("GLUCOSE", cut.Markup, StringComparison.Ordinal);
        });
    }
    ...
    private sealed class FakeDiagnosticsApiClient : IDiagnosticsApiClient
    {
        ...
        {
            return Task.FromResult<PatientDiagnosticTimelineResponse?>(new(
                patientId,
                1,
                [
                    new(
                        ...
                        "Laboratuvar Uzmanı",
                        false,
                        "Rutin panel tamamlandı.",
                        Guid.NewGuid(),
                        [
                            new("GLUCOSE", "98.0", "mg/dL", "70 - 100", "Normal", false),
                        ]),
                ]));
        }
