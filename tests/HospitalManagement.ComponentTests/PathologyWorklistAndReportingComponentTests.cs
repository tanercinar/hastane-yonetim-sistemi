...
public sealed class PathologyWorklistAndReportingComponentTests : BunitContext
{
    private readonly FakeDiagnosticsApiClient _fakeApi = new();

    public PathologyWorklistAndReportingComponentTests()
    {
    ...
    public void PathologyWorklistRendersCasesAndActions()
    {
        var cut = Render<PathologyWorklistAndReporting>();

        cut.WaitForAssertion(() =>
        {
            Assert.Contains("DEMO-PAT-2026-999111", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("Biyopsi", cut.Markup, StringComparison.Ordinal);
            Assert.Contains("Mide Antrum", cut.Markup, StringComparison.Ordinal);
        });
    }
    ...
    private sealed class FakeDiagnosticsApiClient : IDiagnosticsApiClient
    {
        ...
        {
            return Task.FromResult(new List<PathologyCaseSummaryResponse>
            {
                new(
                    Guid.NewGuid(),
                    Guid.NewGuid(),
                    Guid.NewGuid(),
                    "DEMO-PAT-2026-999111",
                    "Biopsy",
                    "Mide Antrum",
                    "GrossExamCompleted",
